...
const Suppliers = () => {
  ...
  const handleDeleteSupplier = async (supplierId: number | undefined) => {
    ...
    if (supplier?.balance !== 0) {
      showError("لا يمكن حذف مورد رصيده غير صفري.");
      return;
    }
    
    if (supplier?.name === 'مورد نقدي') {
      showError("لا يمكن حذف المورد النقدي الافتراضي.");
      return;
    }
                ...
                suppliers.length > 0 ? (
                  suppliers.map((supplier) => (
                    <TableRow key={supplier.id}>
                      ...
                      <TableCell className={`text-left font-mono ${supplier.balance > 0 ? 'text-red-600' : supplier.balance < 0 ? 'text-green-600' : ''}`}>
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="h-24 text-center">
                      لا يوجد موردون. ابدأ بإضافة مورد جديد.
                    </TableCell>
                  </TableRow>
                )
              ) : (
