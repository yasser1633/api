    ...
    if (confirm("هل أنت متأكد من حذف هذه الفاتورة؟ سيتم حذف جميع الدفعات المرتبطة وتحديث رصيد المورد.")) {
      try {
        await db.transaction("rw", db.purchaseInvoices, db.purchaseInvoiceItems, db.suppliers, db.cashTransactions, async () => {
          ...
          const supplier = await db.suppliers.get(invoiceToDelete.supplierId);
          if (supplier) {
            const outstandingAmount = invoiceToDelete.total - invoiceToDelete.paidAmount;
            const newBalance = supplier.balance - outstandingAmount;
            await db.suppliers.update(invoiceToDelete.supplierId, { balance: newBalance });
          }

          // Delete associated cash transactions
          await db.cashTransactions.where({ partyType: 'supplier', partyId: invoiceToDelete.supplierId })
            .and(item => item.description.includes(`PUR-${invoiceToDelete.id?.toString().padStart(3, '0')}`))
            .delete();
