...
const EditPurchaseInvoice = () => {
  ...
  const invoiceData = useLiveQuery(async () => {
    ...
    const items = await db.purchaseInvoiceItems.where({ invoiceId }).toArray();
  ...
  React.useEffect(() => {
    if (invoiceData?.invoice && invoiceData.items) {
      setSelectedSupplierId(invoiceData.invoice.supplierId);
  ...
  const handleUpdateInvoice = async () => {
    if (!selectedSupplierId || !invoiceId) {
    ...
    try {
      await db.transaction("rw", db.purchaseInvoices, db.purchaseInvoiceItems, db.suppliers, async () => {
        ...
        await db.suppliers.update(selectedSupplierId, {
          balance: db.suppliers.get(selectedSupplierId).then(s => (s?.balance || 0) + balanceChange)
        });
        ...
        await db.purchaseInvoiceItems.bulkAdd(newInvoiceItems);
      });
            ...
            <CardContent className="grid gap-6">
              <div className="grid gap-3">
                ...
                <Select value={selectedSupplierId?.toString()} onValueChange={(value) => setSelectedSupplierId(parseInt(value))}>
                  <SelectTrigger id="supplier"><SelectValue placeholder="اختر المورد" /></SelectTrigger>
                  <SelectContent>
