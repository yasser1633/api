...
import { db, PurchaseInvoice, PurchaseInvoiceItem, Item } from "@/lib/db";
...
const EditPurchaseInvoice = () => {
  ...
  const handleSaveInvoice = async () => {
    if (!selectedSupplierId || !invoiceDate || !originalInvoice) {
    ...
    try {
      await db.transaction("rw", db.purchaseInvoices, db.purchaseInvoiceItems, db.suppliers, db.items, async () => {
        ...
        if (originalInvoice.supplierId !== selectedSupplierId) {
            await db.suppliers.where({ id: originalInvoice.supplierId }).modify(s => { s.balance -= (originalTotal - paidAmount) });
            await db.suppliers.where({ id: selectedSupplierId }).modify(s => { s.balance += (newTotal - paidAmount) });
        } else {
        ...
        await db.purchaseInvoiceItems.where({ invoiceId }).delete();
        const invoiceItems = items.map(({ id, ...rest }) => ({ ...rest, invoiceId }));
        await db.purchaseInvoiceItems.bulkAdd(invoiceItems);
      });
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="flex-1 shrink-0 whitespace-nowrap text-xl font-semibold tracking-tight sm:grow-0">
          تعديل فاتورة PUR-{invoiceId.toString().padStart(3, "0")}
        </h1>
        <div className="hidden items-center gap-2 md:ml-auto md:flex">
          <Button variant="outline" size="sm" onClick={() => navigate(`/purchases/${invoiceId}`)}>إلغاء</Button>
          <Button size="sm" onClick={handleSaveInvoice}>حفظ التغييرات</Button>
        </div>
      </div>
